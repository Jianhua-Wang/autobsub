...
class LSFPool:
    ...
    def __init__(
        ...
        self._get_avail_nodes(node_list=node_list)
    ...
    def get_nodes(self, sortByload=True) -> Dict:
        """
        ...
        """
        username = run(["whoami"], stdout=PIPE).stdout.decode("utf-8").strip()
        queue_info = run(["bqueues", "-u", username, "-o", "all", "-json", self.queue], stdout=PIPE).stdout.decode(
            "utf-8"
        )
        queue_info = json.loads(queue_info)["RECORDS"][0]
        hosts = queue_info["HOSTS"].strip().split(" ")
        hosts_info = run(["bhosts", "-o", "all", "-json", *hosts], stdout=PIPE).stdout.decode("utf-8")
        hosts_info = json.loads(hosts_info)["RECORDS"]
        hosts_info = {i["HOST_NAME"]: i for i in hosts_info}

        load_info = run(["lsload", "-o", "HOST_NAME status r1m swp mem", "-json", *hosts], stdout=PIPE).stdout.decode(
            "utf-8"
        )
        load_info = json.loads(load_info)["RECORDS"]
        load_info = {i["HOST_NAME"]: i for i in load_info}
        nodes = {}
        for node in hosts:
            # node, free_mem, memory, avail, cpus, free_cpus, load, partition, state = line.split()
            free_mem = hosts_info[node]["AVAILABLE_MEM"]
            free_mem = float(free_mem.split(" ")[0]) if free_mem != "" else 0
            memory = hosts_info[node]["TOTAL_MEM"]
            memory = float(memory.split(" ")[0]) if memory != "" else 0
            nodes[node] = {
                "free_mem": free_mem,
                "used_mem": memory - free_mem,
                "memory": memory,
                "cpus": int(hosts_info[node]["SLOTS"]),
                "used_cpus": int(hosts_info[node]["SLOTS_ALLOC"]),
                "free_cpus": int(hosts_info[node]["SLOTS"]) - int(hosts_info[node]["SLOTS_ALLOC"]),
                "load": float(load_info[node]["r1m"]),
                "partition": self.queue,
                "state": hosts_info[node]["STATUS"],
            }
        if sortByload:
            nodes = dict(
    ...
    def _get_avail_nodes(
        ...
        # if partition:
        self.nodes = {k: v for k, v in self.nodes.items() if v["partition"] == self.queue}
